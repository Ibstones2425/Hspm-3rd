...
function initDashboard() {
  ...
  .addEventListener("submit",
    handleSendBulkEmail);
  document.getElementById("form-edit-devotional").addEventListener("submit",
    handleEditDevotional);
}
...
async function loadEvents() {
  ...
  snapshot.forEach(docSnap => {
    ...
    ? `<img src="${data.imageUrl}" style="height:100px; width:100%; object-fit:cover; border-radius:8px; margin-bottom:8px;">`: `<div style="height:60px; display:flex; align-items:center; justify-content:center; background:#f5f0f2; border-radius:8px; margin-bottom:8px; color:#aaa;"><i class="fas fa-calendar-alt fa-2x"></i></div>`;
...
async function loadPrayers() {
  ...
  snapshot.forEach(docSnap => {
    ...
    ? new Date(data.date.seconds * 1000).toLocaleDateString(): "N/A";
...
async function loadAnalytics() {
  ...
  try {
    ...
    if (countriesListEl) {
      ...
      const listHtml =
      top15.length === 0
      ? "<p style='color:#aaa; text-align:center;'>No visitor data yet.</p>": top15
      .map(
        ([country, count], i) => `
...
async function loadSubscribers() {
  ...
  try {
    ...
    snapshot.forEach(docSnap => {
      ...
      ? new Date(d.subscribedAt.seconds * 1000).toLocaleDateString(): "N/A";
...
async function handleSendBulkEmail(e) {
  ...
  try {
    ...
    statusEl.innerText =
    failed > 0
    ? `✅ ${sent} sent, ❌ ${failed} failed`: `✅ Successfully sent to all ${sent} subscribers!`;
...
// 2. The submit handler for the update
async function handleEditDevotional(e) {
    e.preventDefault();
    ...
    // Grab the ID and the new values
    const id = document.getElementById('edit-devo-id').value;
    const updatedData = {
        title: document.getElementById('edit-devo-title').value,
        date: document.getElementById('edit-devo-date').value,
        scripture: document.getElementById('edit-devo-scripture').value,
        content: document.getElementById('edit-devo-content').value
    };

    try {
        // Target the specific document in Firestore
        const devoRef = doc(db, "devotionals", id);
        ...
        window.closeModal('modal-edit-devotional');
        loadDevotionals(); 
